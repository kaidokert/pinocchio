...
class SO101Simulation:
    def __init__(self, enable_collision=False):
        ...
        # Initialize Zenoh session for CRCL joint status subscription
        logger.info("Initializing Zenoh session...")
        zenoh_config = zenoh.Config()
        self.zenoh_session = zenoh.open(zenoh_config)
        self.received_joint_statuses = {}  # Store latest received joint statuses
        self.zenoh_message_count = 0
        session_info = self.zenoh_session.info
        logger.info(f"Zenoh session opened: {session_info.zid()}")

        # Subscribe to arm joint status topic
        logger.info("Subscribing to topic: arm/joint_statuses")
        self.joint_status_subscriber = self.zenoh_session.declare_subscriber(
            "arm/joint_statuses",
            self.on_joint_status_received
        )
        logger.info("Zenoh subscriber ready")
        ...
        try:
            ...
            # Zenoh sync configuration
            self.sync_from_zenoh = True  # Enable syncing robot position from Zenoh messages
            self.zenoh_sync_count = 0  # Count of position updates from Zenoh
            self.zenoh_last_msg_time = None  # Timestamp of last received message
            self.zenoh_connection_ok = True  # Assume connection is OK initially
            logger.info(f"Zenoh position sync: {'ENABLED' if self.sync_from_zenoh else 'DISABLED'}")
    ...
    def on_joint_status_received(self, sample):
        """
        Callback for Zenoh subscriber - receives CRCL JointStatusesType messages.

        Args:
            sample: Zenoh sample containing XML-encoded CRCL message
        """
        try:
            self.zenoh_message_count += 1

            # Update connection tracking
            self.zenoh_last_msg_time = time.time()
            self.zenoh_connection_ok = True

            # Extract payload as bytes and parse JSON
            payload = sample.payload.to_bytes()
            payload_str = payload.decode('utf-8')

            # Parse JSON - could be either individual joint or joint_statuses with timestamp
            data = json.loads(payload_str)

            # Handle timestamped message format: {"timestamp": ..., "message": [...]}
            ...
            # Store latest joint statuses by joint number
            for joint_status in joints:
                joint_num = joint_status.get('joint_number')
                if joint_num is None:
                    continue

                joint_pos = joint_status.get('joint_position')
                if joint_pos is None:
                    continue

                self.received_joint_statuses[joint_num] = {
                    'position': joint_pos,
                    'velocity': joint_status.get('joint_velocity'),
                    'torque': joint_status.get('joint_torque_or_force'),
                }

            # Sync robot position if enabled
            if self.sync_from_zenoh and len(self.received_joint_statuses) > 0:
                self.update_robot_from_crcl()

            # Log first few messages for debugging
            if self.zenoh_message_count <= 5:
                logger.info(f"Zenoh message #{self.zenoh_message_count}: Received {len(joints)} joint statuses")
                for joint_num, data in self.received_joint_statuses.items():
                    logger.debug(f"  Joint {joint_num}: pos={data['position']:.4f} rad")
            elif self.zenoh_message_count % 100 == 0:
                # Log every 100th message
                logger.debug(f"Zenoh message #{self.zenoh_message_count}: {len(self.received_joint_statuses)} joints tracked, {self.zenoh_sync_count} syncs")

        except Exception as e:
            logger.error(f"Error processing Zenoh message: {e}", exc_info=True)

    def update_robot_from_crcl(self):
        """
        Update robot configuration from received CRCL joint statuses.
        Maps CRCL joint numbers (1-indexed) to Pinocchio joint indices.

        CRCL joint numbering (1-based):
          1 = shoulder_pan
          2 = shoulder_lift
          3 = elbow_flex
          4 = wrist_flex
        ...
        Pinocchio JOINT_INDICES (0-based, after 6-DOF free flyer):
          [7, 8, 9, 10, 11, 12] for joints 0-5 in JOINT_NAMES
        """
        try:
            # Map CRCL joint numbers to local joint indices (0-based)
            # CRCL uses 1-based indexing, our JOINT_NAMES uses 0-based
            for crcl_joint_num, joint_data in self.received_joint_statuses.items():
                # Convert CRCL 1-based to 0-based index
                joint_idx = crcl_joint_num - 1

                # Validate joint index is in range
                if 0 <= joint_idx < len(JOINT_INDICES):
                    # Get Pinocchio configuration index
                    config_idx = JOINT_INDICES[joint_idx]

                    # Update configuration (CRCL positions are in radians)
                    self.q[config_idx] = joint_data['position']

            self.zenoh_sync_count += 1

            # Enhanced logging - log first few syncs and periodically
            if self.zenoh_sync_count <= 5 or self.zenoh_sync_count % 50 == 0:
                logger.info(f"=== Zenoh Sync #{self.zenoh_sync_count} ===")
                logger.info(f"  Received {len(self.received_joint_statuses)} CRCL joint statuses")
                for crcl_num, data in sorted(self.received_joint_statuses.items()):
                    joint_idx = crcl_num - 1
                    if 0 <= joint_idx < len(JOINT_INDICES):
                        config_idx = JOINT_INDICES[joint_idx]
                        angle_rad = self.q[config_idx]
                        angle_deg = np.degrees(angle_rad)
                        crcl_deg = np.degrees(data['position'])
                        logger.info(f"    CRCL J{crcl_num} ({crcl_deg:7.2f}°) -> q[{config_idx}] = {angle_deg:7.2f}°")

        except Exception as e:
            logger.error(f"Error updating robot from CRCL: {e}", exc_info=True)
    ...
    def shutdown(self):
        """Clean up resources (Zenoh session, etc.)."""
        try:
            if hasattr(self, 'joint_status_subscriber'):
                logger.info("Undeclaring Zenoh subscriber...")
                self.joint_status_subscriber.undeclare()

            if hasattr(self, 'zenoh_session'):
                logger.info("Closing Zenoh session...")
                self.zenoh_session.close()
                logger.info(f"Zenoh session closed. Total messages received: {self.zenoh_message_count}")
        except Exception as e:
    ...
    def draw_ui(self):
        ...
        # Zenoh sync status
        zenoh_status = f"{t.green}ON{t.normal}" if self.sync_from_zenoh else f"{t.normal}OFF{t.normal}"

        # Zenoh connection status with color coding
        if self.zenoh_last_msg_time is None:
            conn_status = f"{t.yellow}WAITING{t.normal}"
            time_since = "no messages yet"
        else:
            time_since_msg = time.time() - self.zenoh_last_msg_time
            if time_since_msg > 5.0:
                conn_status = f"{t.red}TIMEOUT{t.normal}"
                self.zenoh_connection_ok = False
            elif time_since_msg > 2.0:
                conn_status = f"{t.yellow}STALE{t.normal}"
            else:
                conn_status = f"{t.green}ACTIVE{t.normal}"
            time_since = f"{time_since_msg:.1f}s ago"

        lines.append(f"  Zenoh: {conn_status} | Sync: {zenoh_status} | Last: {time_since}")
        lines.append(f"    Messages: {self.zenoh_message_count}, Syncs: {self.zenoh_sync_count}")
    ...
    def toggle_zenoh_sync(self):
        """Toggle Zenoh position synchronization on/off."""
        self.sync_from_zenoh = not self.sync_from_zenoh
        if self.sync_from_zenoh:
            self.add_command("Zenoh Sync ON - Robot will follow real robot")
            logger.info("Zenoh sync enabled")
        else:
            self.add_command("Zenoh Sync OFF - Manual control")
            logger.info("Zenoh sync disabled")
    ...
    def run(self):
        ...
        try:
            with t.fullscreen(), t.cbreak():
                ...
                while self.running:
                    try:
                        ...
                        # Check Zenoh connection timeout
                        if self.zenoh_last_msg_time is not None:
                            time_since_msg = frame_start - self.zenoh_last_msg_time
                            if time_since_msg > 5.0 and self.zenoh_connection_ok:
                                logger.warning(f"Zenoh connection timeout: no messages for {time_since_msg:.1f}s")
                                self.zenoh_connection_ok = False
                        ...
                        if key.name == 'KEY_ESCAPE' or key.lower() == 'q':
                            ...
                            logger.info("Toggle Zenoh sync requested")
                            self.toggle_zenoh_sync()
            ...
            self.shutdown()  # Clean up Zenoh resources
            print(t.clear)
